...
if (negative) {
  cluster.file <- "cluster/priorCneg.RData"
} else {
...
## qsub /spang.compdiag/user/pim28150/cluster/priorC1.sh -l walltime=00:3:00 -q express -o /spang.compdiag/user/pim28150/cluster/temp.out -e /spang.compdiag/user/pim28150/cluster/temp.err
...
for i in `seq 1 6`; do
  ...
  rm -f /spang.compdiag/user/pim28150/cluster/priorC$i.sh
  sleep 5
  done
...
for (n in round(seq(50, length(model$reacID), (length(model$reacID) - 50)/4))) {
  ...
  for (c in 1:10) {
    ...
    if ("cont" %in% type) {
      NEMlist$fc <- NEMlist$fc + rnorm(length(NEMlist$fc), 0, 0.5)
    }
    start <- Sys.time()
...
lines(spec.h.med, type = "b", lty = 3, pch = 3, col = 1, lwd = lwd)
lines(sens.h.med, type = "b", lty = 4, pch = 4, col = 1, lwd = lwd)
...
#legend(1, 0.3, legend = c("sensitivity (ERS)", "specificity (ERS)", "sensitivity (hyper-edges)", "specificity (hyper-edges)"), col = c(1,1,1,1), lty = 1:4, pch = 1:4, lwd = lwd)
