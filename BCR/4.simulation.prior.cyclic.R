...
for (n in round(seq(50, length(model$reacID), (length(model$reacID) - 50)/4))) {
  ...
  for (c in 1:10) {
    ...
    if (length(grep(paste(notnodes, collapse = "|"), rownames(SimResults))) > 0) {
...
## save(CNOlist, CNOresults, CNOinput, file = "superpkn_prior_cycles.RData")
...
pdf("Kube011_BCR_CD40_Inhibitoren/publication/pdf/bioinformatics/gfx/BCR_super_prior_cycles.pdf", width = 5, height = 5)
