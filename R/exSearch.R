...
function(CNOlist,model,sizeFac=10^-10,NAFac=1,NEMlist,parameters=list(cutOffs=c(0,1,0), scoring=c(0.1,0.2,0.9)), parallel = NULL, method = "s", relFit = F, verbose = TRUE, reduce = TRUE, approach = "fc", ...) {
  ...
  getBinScore <- function(j) {
    ...
    scores[j] <- computeScoreNemT1(CNOlist, model = model, pop[j, ], sizeFac = sizeFac, NAFac = NAFac, NEMlist = NEMlist, parameters = parameters, method = method, relFit = relFit,approach=approach)
    return(scores[j])
  }
  if (!is.null(parallel)) {
