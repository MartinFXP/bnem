...
@
The following figure shows the PKN. Red "tee" arrows depict repression the others activation of the child.
...
In a real world application the underlaying real ground truth network (GTN) is not known. However in our toy example we have to define one.
<<definegtn>>=
...
@
The following figure shows our random GTN, which is a subset of the complete search space (based on the PKN).
...
Now we search for the GTN in our full network space. Each network is a sub-graph of the full hyper-graph model\$reacID. We initialise the search with a starting network and greedily search the neighbourhood.
...
On the bottom you see the ERS of S-gene \Sexpr{colnames(CNOlist@signals[[1]])[1]} and the other rows show the observed response scheme (ORS) of the \Sexpr{colnames(CNOlist@signals[[1]])[1]}-regulated E-genes. Even though the Gaussian noise makes the data look almost random, we still found the GTN.

\section*{Stimulated and inhibited S-genes can overlap}
In this section we show you how to use B-NEM when stimuli and inhibitors overlap. For this we allow the PKN to have cycles, but no repression, because repression can lead to an unresolvable ERS. See Pirkl et al., 2016 for details.
...
Same as before we create the full network search space, draw a GTN, simulate data and search for the optimal network.
...
print(sum(ERS.res == ERS)/length(ERS))
@ 
Even though the network looks very different, the accuracy of the ERS is $100\%$.
\end{document}
