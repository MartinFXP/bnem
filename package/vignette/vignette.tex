...
## 	/var/folders/2f/mtjcsd910hz9xnn3shkljg_8005f53/T//RtmpZJv6PZ/downloaded_packages
\end{verbatim}
\begin{alltt}
\hlkwd{library}\hlstd{(devtools)}

\hlkwd{install_github}\hlstd{(}\hlstr{"MartinFXP/B-NEM/package"}\hlstd{,} \hlkwc{force} \hlstd{= T)}
\end{alltt}


{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Downloading GitHub repo MartinFXP/B-NEM@master\\\#\# from URL https://api.github.com/repos/MartinFXP/B-NEM/zipball/master}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Installing bnem}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# '/Library/Frameworks/R.framework/Resources/bin/R' --no-site-file --no-environ --no-save\ \ \textbackslash{}\\\#\#\ \  --no-restore --quiet CMD INSTALL\ \ \textbackslash{}\\\#\#\ \  '/private/var/folders/2f/mtjcsd910hz9xnn3shkljg\_8005f53/T/RtmpZJv6PZ/devtools5a606bfccea6/MartinFXP-B-NEM-17ba544/package'\ \ \textbackslash{}\\\#\#\ \  --library='/Users/mpirkl/Library/R/3.3/library' --install-tests}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# }}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Reloading installed bnem}}\begin{alltt}
...
\hlcom{## to get the while loop started,}
\hlcom{## which makes sure we get a PKN with exactly two stimuli (or what the amount you want):}
\hlstd{stimuli} \hlkwb{<-} \hlstr{"dummy"}

\hlkwa{while}\hlstd{(}\hlkwd{length}\hlstd{(stimuli)} \hlopt{!=} \hlnum{2}\hlstd{) \{}

\hlcom{## random Boolean graph without cycles,}
\hlcom{## maximal 25 edges, maximal edge size of 1 (normal graph) and maximal 10 S-genes:}
...
\hlkwa{for} \hlstd{(i} \hlkwa{in} \hlstd{dnf) \{}
  ...
  \hlstd{\}}
\hlstd{\}}
\hlkwd{write.table}\hlstd{(sifMatrix,} \hlkwc{file} \hlstd{=} \hlstr{"temp.sif"}\hlstd{,} \hlkwc{sep} \hlstd{=} \hlstr{"\textbackslash{}t"}\hlstd{,}
            \hlkwc{row.names} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlkwc{col.names} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlkwc{quote} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\hlstd{PKN} \hlkwb{<-} \hlkwd{readSIF}\hlstd{(}\hlstr{"temp.sif"}\hlstd{)}
\hlcom{## unlink("temp.sif")}

\hlcom{## create metainformation (which S-genes are perturbed in which experiments):}
\hlstd{CNOlist} \hlkwb{<-} \hlkwd{dummyCNOlist}\hlstd{(}\hlkwc{stimuli} \hlstd{= stimuli,} \hlkwc{inhibitors} \hlstd{= inhibitors,}
                        \hlkwc{maxStim} \hlstd{=} \hlnum{2}\hlstd{,} \hlkwc{maxInhibit} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{signals} \hlstd{=} \hlkwa{NULL}\hlstd{)}
...
\hlcom{## we find constitutively active S-genes with the following (the are boring, so we avoid them):}
\hlstd{ind} \hlkwb{<-} \hlkwd{grep}\hlstd{(}\hlkwd{paste}\hlstd{(inhibitors,} \hlkwc{collapse} \hlstd{=} \hlstr{"|"}\hlstd{),} \hlkwd{colnames}\hlstd{(steadyState2))}
\hlstd{steadyState2[, ind]} \hlkwb{<-} \hlstd{steadyState2[, ind]} \hlopt{+} \hlstd{CNOlist}\hlopt{@}\hlkwc{inhibitors}

\hlcom{## this while loop makes sure we get a gtn,}
\hlcom{## which actually affects all vertices}
\hlcom{## and no vertices are constitutively active:}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## the expression data with 10 E-genes for each S-gene and 3 replicates:}
...
\hlcom{## we calculate the foldchanges (expected S-gene response scheme, ERS)}
\hlcom{## between certain condtions (e.g. control vs stimulation):}
\hlstd{ERS} \hlkwb{<-} \hlkwd{computeFc}\hlstd{(CNOlist,} \hlkwd{t}\hlstd{(steadyState))}

\hlcom{# the next step reduces the ERS to a sensible set of comparisons;}
\hlcom{## e.g. we do not want to compare stimuli vs inhibition, but stimuli vs (stimuli,inhibition):}
\hlstd{stimcomb} \hlkwb{<-} \hlkwd{apply}\hlstd{(}\hlkwd{expand.grid}\hlstd{(stimuli, stimuli),} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{2}\hlstd{), as.character)}
\hlstd{stimuli.pairs} \hlkwb{<-} \hlkwd{apply}\hlstd{(stimcomb,} \hlnum{1}\hlstd{, paste,} \hlkwc{collapse} \hlstd{=} \hlstr{"_"}\hlstd{)}

\hlcom{## this is the usual setup,}
\hlcom{## but "computeFc" calculates a lot more contrasts, which can also be used, if preferred:}
\hlstd{ind} \hlkwb{<-} \hlkwd{grep}\hlstd{(}\hlkwd{paste}\hlstd{(}\hlkwd{c}\hlstd{(}\hlkwd{paste}\hlstd{(}\hlstr{"Ctrl_vs_"}\hlstd{,} \hlkwd{c}\hlstd{(stimuli, inhibitors),} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{),}
                    \hlkwd{paste}\hlstd{(stimuli,} \hlstr{"_vs_"}\hlstd{, stimuli,} \hlstr{"_"}\hlstd{,} \hlkwd{rep}\hlstd{(inhibitors,} \hlkwc{each} \hlstd{=} \hlkwd{length}\hlstd{(stimuli)),} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{),}
                    \hlkwd{paste}\hlstd{(stimuli.pairs,} \hlstr{"_vs_"}\hlstd{, stimuli.pairs,} \hlstr{"_"}\hlstd{,} \hlkwd{rep}\hlstd{(inhibitors,} \hlkwc{each} \hlstd{=} \hlkwd{length}\hlstd{(stimuli.pairs)),} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{)),}
                  \hlkwc{collapse} \hlstd{=} \hlstr{"|"}\hlstd{),} \hlkwd{colnames}\hlstd{(ERS))}
\hlstd{ERS} \hlkwb{<-} \hlstd{ERS[, ind]}
...
\hlcom{## in real applications some E-genes are negatively regulated, hence we flip some foldchanges:}
...
\hlcom{## don't forget to set rownames (usually gene symbols, ensemble ids, entrez ids, ...)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## we start with the empty graph:}
...
\hlcom{## initBstring <- reduceGraph(rep(1, length(model$reacID)), model, CNOlist) ## fully connected normal graph}

\hlcom{## paralellize for several threads on one machine or multiple machines}
\hlcom{## see package "snowfall" for details}
\hlstd{parallel} \hlkwb{<-} \hlnum{2}
\hlcom{## parallel <- list(c(4,16,8,2), c("machine1", "machine2", "machine3", "machine4")) # distribute 30 threads on four machines}
...
\hlstd{greedy} \hlkwb{<-} \hlkwd{bnem}\hlstd{(}\hlkwc{search} \hlstd{=} \hlstr{"greedy"}\hlstd{,}
            ...
            \hlkwc{draw} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlcom{# TRUE: draw network at each step}
            \hlkwc{verbose} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlcom{# TRUE: print changed (hyper-)edges and score improvement}
...
\hlkwd{print}\hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{1} \hlopt{&} \hlstd{resString} \hlopt{==} \hlnum{1}\hlstd{)}\hlopt{/}
      \hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{1} \hlopt{&} \hlstd{resString} \hlopt{==} \hlnum{1}\hlstd{)} \hlopt{+} \hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{1} \hlopt{&} \hlstd{resString} \hlopt{==} \hlnum{0}\hlstd{)))}
\end{alltt}
\begin{verbatim}
## [1] 1
\end{verbatim}
\begin{alltt}
\hlkwd{print}\hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{resString} \hlopt{==} \hlnum{0}\hlstd{)}\hlopt{/}
      \hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{resString} \hlopt{==} \hlnum{0}\hlstd{)} \hlopt{+} \hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{resString} \hlopt{==} \hlnum{1}\hlstd{)))}
\end{alltt}
\begin{verbatim}
## [1] 1
\end{verbatim}
\begin{alltt}
\hlcom{## accuracy of the expected response scheme (can be high even, if the networks differ):}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fitinfo} \hlkwb{<-} \hlkwd{validateGraph}\hlstd{(CNOlist,} \hlkwc{fc}\hlstd{=fc,} \hlkwc{exprs}\hlstd{=exprs,} \hlkwc{model} \hlstd{= model,} \hlkwc{bString} \hlstd{= resString,}
                         \hlkwc{Sgene} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{Egenes} \hlstd{=} \hlnum{1000}\hlstd{,} \hlkwc{cexRow} \hlstd{=} \hlnum{0.8}\hlstd{,} \hlkwc{cexCol} \hlstd{=} \hlnum{0.7}\hlstd{,} \hlkwc{xrot} \hlstd{=} \hlnum{45}\hlstd{,}
                         \hlkwc{Colv} \hlstd{= T,} \hlkwc{Rowv} \hlstd{= T,} \hlkwc{dendrogram} \hlstd{=} \hlstr{"both"}\hlstd{,} \hlkwc{bordercol} \hlstd{=} \hlstr{"lightgrey"}\hlstd{,}
                         \hlkwc{aspect} \hlstd{=} \hlstr{"iso"}\hlstd{,} \hlkwc{sub} \hlstd{=} \hlstr{""}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] "1.B: 13"
## [1] "Activated: 6"
...
B and the other rows show the
observed response scheme (ORS) of the
B-regulated E-genes. Even
...
\hlstd{exprs} \hlkwb{<-} \hlkwd{t}\hlstd{(steadyState)[}\hlkwd{rep}\hlstd{(}\hlnum{1}\hlopt{:}\hlkwd{ncol}\hlstd{(steadyState),} \hlnum{10}\hlstd{),} \hlkwd{rep}\hlstd{(}\hlnum{1}\hlopt{:}\hlkwd{nrow}\hlstd{(steadyState),} \hlnum{3}\hlstd{)]}
\hlstd{ERS} \hlkwb{<-} \hlkwd{computeFc}\hlstd{(CNOlist,} \hlkwd{t}\hlstd{(steadyState))}
\hlstd{stmcomb} \hlkwb{<-} \hlkwd{apply}\hlstd{(}\hlkwd{expand.grid}\hlstd{(stimuli, stimuli),} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{2}\hlstd{), as.character)}
\hlstd{stimuli.pairs} \hlkwb{<-} \hlkwd{apply}\hlstd{(stimcomb,} \hlnum{1}\hlstd{, paste,} \hlkwc{collapse} \hlstd{=} \hlstr{"_"}\hlstd{)}
\hlstd{ind} \hlkwb{<-} \hlkwd{grep}\hlstd{(}\hlkwd{paste}\hlstd{(}\hlkwd{c}\hlstd{(}\hlkwd{paste}\hlstd{(}\hlstr{"Ctrl_vs_"}\hlstd{,} \hlkwd{c}\hlstd{(stimuli, inhibitors),} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{),}
                    \hlkwd{paste}\hlstd{(stimuli,} \hlstr{"_vs_"}\hlstd{, stimuli,} \hlstr{"_"}\hlstd{,} \hlkwd{rep}\hlstd{(inhibitors,} \hlkwc{each} \hlstd{=} \hlkwd{length}\hlstd{(stimuli)),} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{),}
                    \hlkwd{paste}\hlstd{(stimuli.pairs,} \hlstr{"_vs_"}\hlstd{, stimuli.pairs,} \hlstr{"_"}\hlstd{,} \hlkwd{rep}\hlstd{(inhibitors,} \hlkwc{each} \hlstd{=} \hlkwd{length}\hlstd{(stimuli.pairs)),} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{)),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{print}\hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{1} \hlopt{&} \hlstd{resString2} \hlopt{==} \hlnum{1}\hlstd{)}\hlopt{/}
      \hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{1} \hlopt{&} \hlstd{resString2} \hlopt{==} \hlnum{1}\hlstd{)} \hlopt{+} \hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{1} \hlopt{&} \hlstd{resString2} \hlopt{==} \hlnum{0}\hlstd{)))}
\end{alltt}
\begin{verbatim}
## [1] 0.6666667
\end{verbatim}
\begin{alltt}
\hlkwd{print}\hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{resString2} \hlopt{==} \hlnum{0}\hlstd{)}\hlopt{/}
      \hlstd{(}\hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{resString2} \hlopt{==} \hlnum{0}\hlstd{)} \hlopt{+} \hlkwd{sum}\hlstd{(bString} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{resString2} \hlopt{==} \hlnum{1}\hlstd{)))}
\end{alltt}
\begin{verbatim}
## [1] 0.9824561
\end{verbatim}
...
##  [1] bnem_1.0            latticeExtra_0.6-28 RColorBrewer_1.1-2  lattice_0.20-33    
...
## [16] withr_1.0.2      httr_1.2.1       stringr_1.1.0    stats4_3.3.1     R6_2.1.3        
## [21] plotrix_3.6-3    tcltk_3.3.1      limma_3.28.20    magrittr_1.5     scales_0.4.0    
