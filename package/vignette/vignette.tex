...
## 	/var/folders/2f/mtjcsd910hz9xnn3shkljg_8005f53/T//RtmprQnyC4/downloaded_packages
\end{verbatim}
\begin{alltt}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# '/Library/Frameworks/R.framework/Resources/bin/R' --no-site-file --no-environ --no-save\ \ \textbackslash{}\\\#\#\ \  --no-restore --quiet CMD INSTALL\ \ \textbackslash{}\\\#\#\ \  '/private/var/folders/2f/mtjcsd910hz9xnn3shkljg\_8005f53/T/RtmprQnyC4/devtools3ef64d54e11b/MartinFXP-B-NEM-560f7d6/package'\ \ \textbackslash{}\\\#\#\ \  --library='/Users/mpirkl/Library/R/3.3/library' --install-tests}}
...
\hlcom{## which makes sure we get a PKN with exactly two stimuli (not necessary):}
...
\hlcom{## and no vertices are constitutively active (not necessary):}
...
\hlcom{## we calculate the foldchanges or expected S-gene response scheme (ERS)}
\hlcom{## between certain condtion (e.g. control vs stimulation):}
...
\end{verbatim}
\begin{alltt}
\hlcom{## accuracy of expected response scheme from learned network should be high}
\hlcom{## even though the network can look different:}
...
\hlstd{fitinfo} \hlkwb{<-} \hlkwd{validateGraph}\hlstd{(CNOlist,} \hlkwc{fc}\hlstd{=fc,} \hlkwc{exprs}\hlstd{=exprs,} \hlkwc{model} \hlstd{= model,} \hlkwc{bString} \hlstd{= resString,}
                         ...
                         \hlkwc{Colv} \hlstd{= T,} \hlkwc{Rowv} \hlstd{= T,} \hlkwc{dendrogram} \hlstd{=} \hlstr{"both"}\hlstd{,} \hlkwc{bordercol} \hlstd{=} \hlstr{"lightgrey"}\hlstd{,} \hlkwc{aspect} \hlstd{=} \hlstr{"iso"}\hlstd{,} \hlkwc{sub} \hlstd{=} \hlstr{""}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] "1.B: 13"
## [1] "Activated: 6"
...
\end{kframe}
\end{knitrout}
The optimal network looks a bit different and has lower sensitivity and specificity. However the accuracy of the ERS is still $100\%$.
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
