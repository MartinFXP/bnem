...
\section*{B-Cell receptor signalling}
...
print(min(greedy$scores))
@
<<plotresultbcr, fig.width=9, fig.height=6, out.width='0.6\\linewidth'>>=
par(mfrow=c(1,2))
plotDnf(, main = "PKN", stimuli = "BCR")
plotDnf(ga$graph, main = "genetic optimum", stimuli = "BCR")
plotDnf(greedy$graph, main = "greedy optimum", stimuli = "BCR")
@

The greedy search improves the result of the genetic algorithm and
Pi3k is not alternatively regulating p38 anymore together with Ikk2.
