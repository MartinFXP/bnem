...
while(length(stimuli) == 1) { # we want at least two stimulations

  dnf <- randomDnf(10, max.edges = 25, max.edge.size = 1, dag = TRUE) # create random disjunctive normal form (dnf) of 10 literals (=vertices, S-genes) with at max 1 parent per edge and 25 edges and no cycles
...
steadyState <- simulateStatesRecursive(CNOlist, model, bString) # we simulate the steady states for all possible conditions

while(any(apply(steadyState, 2, sd) == 0)) { # this while loop makes sure we get a gtn which actually affects all vertices:
...
plotDnf(model$reacID[as.logical(bString)]) # let's look at our gtn
...
method <- "s" # spearman rank correlation scores E-genes S-gene association/similarity
parameters <- list(cutOffs = c(0,1,0), scoring = c(0.13,0.05,0.25)) # if method = NULL, these are different parameters for other association scores (e.g. marginal likelihood). cutOffs[3] is used for internal E-gene selection. Leave that at 0, if you don't know what that does. For more information look for the dissertation of Martin Pirkl, University of Regensburg.
sizeFac <- 10^-10 # zeta size penalty for network complexity (usually low but none zero, if you have a rigorous E-gene selection)

parallel <- 2 # list(c(1,2,3,4), c("machine1", "machine2", "machine3", "machine4")) # parallel calculation of edge improvements

res <- localSearch(CNOlist=CNOlist, NEMlist=NEMlist, model=model, parallel=parallel, parallel2=1, seeds=1, initSeed = initBstring, sizeFac = sizeFac, method = method, verbose = verbose, draw = draw, max.steps = Inf, parameters = parameters) # this is the greedy search instead of the genetic algorithm
...
for (i in 1:ncol(CNOlist@signals[[1]])) {
  geneLists[[i]] <- validateGraph(CNOlist, NEMlist, model = model, bString = res$bStrings[1, ], Sgene = i, Egenes = 10, parameters=parameters, cexRow = 0.8, soft = T, cexCol = 0.7, xrot = 45, method = method, disc = 0, Colv = T, Rowv = T, dendrogram = "both", bordercol = "grey", aspect = "iso", sub = "")
