...
sim <- simBoolGtn(Sgenes = 5, maxEdges = 10, keepsif = TRUE, sd = 0.5)
fc <- sim$fc
exprs <- sim$exprs
CNOlist <- sim$CNOlist
model <- sim$model
...
```{r}
set.seed(1337)
sim <- simBoolGtn(Sgenes = 4, maxEdges = 10, dag = FALSE,
                  negation = FALSE, allstim = TRUE)
fc <- sim$fc
exprs <- sim$exprs
CNOlist <- sim$CNOlist
model <- sim$model
...
Four nodes switched places between the two results. However, the 
PKN start gives a better score at `r min(bcrRes[[1]]$scores[[1]])` 
to `r min(bcrRes[[1]]$scores[[1]])` for the empty start.
