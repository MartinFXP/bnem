...
data(bnemdata)
#load("~/Dokumente/B-NEM/data/bnem_data.rda")

greedy <- bnemdata$greedy
genetic <- bnemdata$genetic
greedy2 <- bnemdata$greedy2
greedy3 <- bnemdata$greedy3
greedy2b <- bnemdata$greedy2b
exhaustive <- bnemdata$exhaustive
bcr_greedy <- bnemdata$bcr_greedy
bcr_genetic <- bnemdata$bcr_genetic
...
The following function creates a directed acyclic graph (dag), which is then extended to a full Boolean network. From this network we sample a subset of edges to define our groudn truth. In the last step, the function generates data from the ground truth.
...
sim <- simBoolGtn(n = 5, e = 15, keepsif = TRUE)
fc <- sim$fc
exprs <- sim$exprs
CNOlist <- sim$CNOlist
model <- sim$model
...
The following figure shows the PKN and the ground truth. Red "tee" arrows depict repression the others activation of the child. The stimulated S-genes are diamond shaped.
...
We suggest to take a look at the sif file in the workign directory. In future analyses it is easier to just provide a suitable sif file for the investigated pathway. In an real application the underlaying ground truth network (GTN) is not known.
...
We search for the GTN in our restricted network space. Each network is a sub-graph of the full hyper-graph sim\$model\$reacID. We initialise the search with a starting network and greedily search the neighbourhood.

We use two different network scores. "s" is the rank correlation and "llr" is just the euclidean dot product (hence no shift and scale invariance).
...
```{r}
set.seed(1337)
sim <- simBoolGtn(n = 5, e = 15, dag = FALSE, negation = FALSE, allstim = TRUE)
fc <- sim$fc
exprs <- sim$exprs
CNOlist <- sim$CNOlist
model <- sim$model
